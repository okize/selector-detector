...
var prettyPrint = function (json) {
  ...
  var table = new Table({ head: ['Stylesheet Name', 'Rules', 'Selectors', 'Declarations'] });
  ...
  function pushRowsToTable (el, i, arr) {

    var name, rules, selectors, declarations;

    name = resultArr[i].name,
    rules = resultArr[i].rules,
    selectors = checkCount(resultArr[i].selectors),
    declarations = resultArr[i].declarations;

    table.push([name, rules, selectors, declarations]);

  }

  resultArr.forEach(pushRowsToTable);
