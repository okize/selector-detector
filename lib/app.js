...
module.exports = function (userUrl, pretty) {
  ...
  request(requestOpts, function (error, response, body) {
    ...
    // check for ie
    var headNodes = $('head')[0].children;

    // regex for testing ie conditional comments
    var regex = /\[if.+]>.+<\!\[endif\]/g;

    // var ieStylesheets;

    for (var k = 0; k < headNodes.length; k++) {
      if (headNodes[k].type === 'comment') {

        // strip whitespace
        var foo = headNodes[k].data.replace(/\s+/g, '');

        // check if it seems to be IE conditional comments
        if (foo.match(regex)) {

          $$ = cheerio.load(headNodes[k].data);
